...
namespace SeppePeelman.EditorTools.SurfaceAlignTool
{
    ...
    public class SurfaceAlignTool : EditorTool
    {
        ...
        public override void OnToolGUI(EditorWindow window)
        {
            ...
            // Rotation handle
            Handles.color = _upAxisColor;
            targetRotation = Handles.Disc(2, targetRotation, _activeTransform.position, GetUpAxis(), HandleUtility.GetHandleSize(_activeTransform.position) * 1.2f, false, 15);

            // Random rotation
            bool applyRandomRotationNow = false;    
            if (_settings.RotateRandomAfterMoving && Event.current.type == EventType.Used && _previousGuiHotcontrol == 0 && (GUIUtility.hotControl != 2 && _previousGuiHotcontrol != 2))
            {
                applyRandomRotationNow = true;
                GUI.changed = true;
            }

            // Scale Handle
            Handles.color = Color.white;
            Vector3 scaleHandlePosition = _activeTransform.position + (Vector3.up + Camera.current.transform.right) * 0.5f * HandleUtility.GetHandleSize(_activeTransform.position);
            float targetScaleX = Handles.ScaleValueHandle(_activeTransform.localScale.x, scaleHandlePosition, _activeTransform.rotation, HandleUtility.GetHandleSize(_activeTransform.position), Handles.CubeHandleCap, 0);

            float scaleDelta = targetScaleX - _activeTransform.localScale.x;

            // Apply transform changes
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(_activeTransform, "Move " + _activeTransform.name);
                if (applyRandomRotationNow)
                {
                    ApplyRandomRotation();
                }
                else
                {
                    ...
                    _activeTransform.localScale = new Vector3(
                        _activeTransform.localScale.x + scaleDelta,
                        _activeTransform.localScale.y + scaleDelta,
                        _activeTransform.localScale.z + scaleDelta
                        );
                }
        ...
        private static void DoOverlayUI(UnityEngine.Object settingsObject, SceneView sceneView)
        {
            ...
            settings.RotateRandomAfterMoving = EditorGUILayout.Toggle("Rotate Random:", settings.RotateRandomAfterMoving);
        }
#endif
    }
